#### plot_score (SummarizedExperiment) ####

#' @rdname plot_score
#' @export
setMethod("plot_score", signature(x = "SummarizedExperiment"), 
          function(x,
                   model.c = "",
                   components.vi = c(1, 2),
            ...
            if (model.c %in% names(ropls_model.ls)) {
              ropls.model <- ropls_model.ls[[model.c]]
            } else
              stop(paste0("'", model.c, "' was not found in the models from the provided SummarizedExperiment: '", paste(names(ropls_model.ls), collapse = ", "), "'"))
...
#### plot_score (opls generated from an ExpressionSet) ####

#' @rdname plot_score
#' @export
setMethod("plot_score", signature(x = "opls"),
          function(x,
                   model.c = "",
                   components.vi = c(1, 2),
                   label.c = c("", "sample_names")[2],
            ...
            eset <- getEset(x)
            if (any(dim(eset) < 1))
              stop("ExpressionSet object could not be extracted from the 'x' argument. Note that 'plot_score' application to an 'opls' object is currently only implemented for objects generated by applying the 'opls' modeling to an ExpressionSet.")
