...
## Working on *SummarizedExperiment* omics objects from bioconductor
...
intensities, and two DataFrame containing the sample and variable
metadata, which can be accessed by the `assay`, `colData` and `rowData`
methods respectively (note that the data matrix is stored in the object
...
used as the response. It returns the updated **`SummarizedExperiment`**
object with the loading, score, VIP, etc. data as new columns in the
`colData` and `rowData`, and with the PCA/(O)PLS(-DA) models in the
`metadata` slot.

In the example below, we will first build a minimal
**`SummarizedExperiment`** object from the *sacurine* data set, and we
subsequently perform a PLS-DA.

We first build the `SummarizedExperiment` object:

```{r se_building, message = FALSE, warning=FALSE}
...
Note: Application of `opls` to `ExpressionSet` objects is deprecated.

## Working with multi-omics datasets: the MultiAssayExperiment format
...
dataset available in the `omicade4` package.

Loading the *NCI60_4arrays* from the `omicade4` package and selecting
...
```{r mae_building, message = FALSE, warning=FALSE}
library(MultiAssayExperiment)
# Building the SummarizedExperiment instances
experiment.ls <- list()
sampleMap.ls <- list()
...
# The sample metadata are stored in the colData data frame
...
Building a **PCA** model on each dataset:
...
Note: Application of `opls` to `MultiDataSet` objects is deprecated.
