...
## Outliers: computing the orthogonal (OD) and score distances (SD)

We show below the code used in [ropls](http://bioconductor.org/packages/release/bioc/html/ropls.html) to compute the distances within and orthogonal to the projection
plane (respectively SD and OD) to detect outliers [@Hubert2005]. The example is illustrated in the case of a PLS-DA model:

```{r SD_OD}
model <- sacurine.plsda

typeVc <- "outlier"
parCompVi <- 1:2

# Getting the scores and loadings

summaryDF <- getSummaryDF(model)
scoreMN <- getScoreMN(model)
loadingMN <- getLoadingMN(model)

if (summaryDF[, "pre"] + summaryDF[, "ort"] < 2) {
  ...
  if (!all(typeVc %in% c("permutation", "overview"))) {
    ...
    typeVc <- "overview"
    if (!is.null(model@suppLs[["permMN"]]))
  ...
  if (summaryDF[, "ort"] > 0) {
    if (parCompVi[2] > summaryDF[, "ort"] + 1)
    ...
    tCompMN <- cbind(scoreMN[, 1], model@orthoScoreMN[, parCompVi[2] - 1])
    pCompMN <- cbind(loadingMN[, 1], model@orthoLoadingMN[, parCompVi[2] - 1])
    colnames(pCompMN) <- colnames(tCompMN) <- c("h1", paste("o", parCompVi[2] - 1, sep = ""))
  } else {
    if (max(parCompVi) > summaryDF[, "pre"])
...
# Computing the score and orthogonal distances
#  Note that the score distance relies on the Mahalanobis distance
...
pcaResMN <- cbind(sdsVn = apply(tCompMN,
                  ...
                  odsVn = apply(model@suppLs[["xModelMN"]] - tcrossprod(tCompMN, pCompMN),
                                1,
                                function(x) sqrt(drop(crossprod(x[complete.cases(x)])))))

# Defining the thresholds for both metrics
...
if (!is.null(model@suppLs[["yMCN"]]) && ncol(model@suppLs[["yMCN"]]) == 1) { ## (O)PLS of single response
  obsLabVc <- rownames(model@suppLs[["yMCN"]])
} else {## PCA
