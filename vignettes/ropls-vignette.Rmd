...
sac.plsda <- opls(sac.set, "gender", fig.pdfC = "none", info.txtC = "none")
```

Note that this time `opls` returns the model as an object of the `opls`
class. The update `ExpressionSet` object can be accessed with the
`getEset` method:
...
dataset from the `omicade4` package (which is has been made available in
...
MultiAssayExperiment::colData(nci.mae[["agilent"]])
```
...
The `MultiDataSet` format is currently supported as a legacy
representation from the previous versions of the `ropls` package
(\<1.28.0) but will now be supplanted by `MultiAssayExperiment` in
future versions.
...
`phenomis` package. The `phenomis` package is currently available on
...
resource for computational metabolomics, based on the Galaxy
environment.
